...
pub fn process_trade_instruction(
    ...
    let signature = transaction_obj.get("transaction").unwrap().get("signatures").unwrap().get(0).unwrap().as_str().unwrap().to_string();
    ...
    let bonding_curve_token_post_balance = BigUint::from_str_radix(
        bonding_curve_token_balances.post_balances[0].get("uiTokenAmount").unwrap().get("amount").unwrap().as_str().unwrap(),
        10,
    ).unwrap();
    let bonding_curve_sol_post_balance = BigUint::from_str_radix(
        bonding_curve_sol_balances.post_balances[0].as_str().unwrap(),
        10,
    ).unwrap();
