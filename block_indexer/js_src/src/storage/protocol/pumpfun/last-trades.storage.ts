...
export interface IPumpfunLastTradeValues {
  timestamp: number;
  isBuy: boolean;
  tokenAmount: bigint;
  solAmount: bigint;
...
export async function saveLastTrades(
  ...
  const tradeEventsMints = [
    ...new Set(tradeEvents.map((event) => event.eventObj.mint.toBase58())),
  ];
  ...
  const mintToTradesMap: { [mint: string]: IPumpfunLastTradeValues[] } = {};
  ...
  for (const tradeEvent of tradeEvents) {
    ...
    const value: IPumpfunLastTradeValues = {
